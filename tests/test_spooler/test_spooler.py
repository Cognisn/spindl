...
class TestResponseSpooler:
    async def test_not_initialised_raises(self, spooler_config):
        s = ResponseSpooler(spooler_config)
        with pytest.raises(RuntimeError, match="not initialised"):
            await s.process_response(response={"data": []}, source_tool="test")
    ...
    async def test_small_array_inline(self, spooler):
    ...
    async def test_large_array_spooled(self, spooler):
    ...
    async def test_auto_detect_arrays(self, spooler):
    ...
    async def test_list_response_wrapped(self, spooler):
        items = [{"id": i} for i in range(50)]
        result = await spooler.process_response(response=items, source_tool="test")
        assert "spooled_data" in result

    async def test_spool_summary_has_sample(self, spooler):
        items = [{"id": i, "name": f"item-{i}"} for i in range(50)]
        result = await spooler.process_response(
            response={"items": items},
            source_tool="test",
    ...
    async def test_spool_summary_has_columns(self, spooler):
        items = [{"id": i, "name": f"item-{i}"} for i in range(50)]
        result = await spooler.process_response(
            response={"items": items},
            source_tool="test",
    ...
    async def test_guidance_uses_placeholders(self, spooler):
        items = [{"id": i} for i in range(50)]
        result = await spooler.process_response(
            response={"items": items},
            source_tool="test",
    ...
    async def test_get_connection(self, spooler):
        conn = spooler.get_connection()
        assert conn is not None

    async def test_get_connection_not_initialised(self, spooler_config):
        s = ResponseSpooler(spooler_config)
